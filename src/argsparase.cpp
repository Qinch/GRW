...
char* const short_options = (char* const)"m:p:an:c:h";
...
inline void hint(char *app)
{
	...
	printf("    例如:\n\t1 %s -m ./data/model1_1.in -p ./data/powermap1.in -a;\n \
			\r\t2 %s -m ./data/model1_1.in -p ./data/powermap1.in -a -n 300;\n \
			\r\t3 %s -m ./data/model1_1.in -p ./data/powermap1.in -c [x1,y1]\n \
			\r\t4 %s -m ./data/model1_1.in -p ./data/powermap1.in -c [x1,y1] -c [x2,y2]\n \
			\r\t5 %s -m ./data/model1_1.in -p ./data/powermap1.in -c [x1,y1],[x2,y2],[x3,y3]\n", \
											app,app,app,app,app);
}
...
{      
	...
	if(  !( (args_m&&args_p&&args_a&&(!args_c)) || (args_m&&args_p&&args_a&&args_n&&(!args_c)) \
		|| (args_m&&args_p&&args_c&&(!args_a)&&(!args_n)) )  )//参数设置错误;
	{
